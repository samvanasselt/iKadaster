...
**--------------------------------------------------------------
**    NLTRANS2008
**    Based on NLTRANS2004.
**    Authors: Jochem Lesparre, Joop van Buren, Marc Crombaghs, Frank Dentz, Arnoud Pol, Sander Oude Elberink
**    Some slight modifications for the Code::Blocks compiler
**    7 Parameters similarity transformation modified
**    0.0088 offset in etrs2nap and nap2etrs
**    Point id's added.
**    Output also outside grid validity areas.
**    Indicators behind point ID:
**     (space) RD and geoid grids available
**    - RD grid not available
**    | geoid grid not available
**    + RD and geoid not available
**--------------------------------------------------------------
*/
            ...
            double& phi_etrs, double& lambda_etrs, double& h_etrs)
{
    ...
    **    The error made by using a constant for de Bessel geoid height
    **    is circa 1 meter maximum in the ellipsoidal height
    **    (for the NLGEO2004 geoid model). This intoduces an error in
    **    the phi, lambda position too, this error is nevertheless
    **    certainly smaller than 0.0001 m.
    **--------------------------------------------------------------
    */
...
/*
**--------------------------------------------------------------
**    Function name: bessel2etrs
**--------------------------------------------------------------
*/
void bessel2etrs(double phi_bessel, double lambda_bessel, double h_bessel,
                 double& phi_etrs, double& lambda_etrs, double& h_etrs)
{
    double x_bessel, y_bessel, z_bessel;
    double x_etrs, y_etrs, z_etrs;
             ...
             double& nap)
{
    ...
    **    Explanation of the meaning of variables:
    **        n  geoid height
    **        0.0088 offset to compensate for AGRS 2008 heights
    **--------------------------------------------------------------
    */
    int error;
    double n;
...
{
    int error;
    double h_etrs_sim, h_etrs;
    error = rd2etrs( x_rd, y_rd,   nap, phi, lambda, h_etrs_sim);
    error = nap2etrs(phi,  lambda, nap, h_etrs);
...
**--------------------------------------------------------------
*/
int output(string stat_id, double phi_etrs, double lambda_etrs, double h_etrs)
{
    int error_rd=0;
    int error_nap=0;
	int deg_phi_etrs, min_phi_etrs, deg_lambda_etrs, min_lambda_etrs;
    int deg_phi_bessel, min_phi_bessel, deg_lambda_bessel, min_lambda_bessel;
	double sec_phi_etrs, sec_lambda_etrs;
	double sec_phi_bessel, sec_lambda_bessel;
    double x_etrs, y_etrs, z_etrs;
    double x_amersfoort_bessel, y_amersfoort_bessel, z_amersfoort_bessel;
    double x_amersfoort_etrs, y_amersfoort_etrs, z_amersfoort_etrs;
    double x_bessel, y_bessel, z_bessel;
	double phi_bessel, lambda_bessel, h_bessel;
    double x_pseudo_rd, y_pseudo_rd, x_rd, y_rd, nap, h_geoid;
    ...
    **--------------------------------------------------------------
    */
    decimal2deg_min_sec(phi_etrs, deg_phi_etrs, min_phi_etrs, sec_phi_etrs);
    decimal2deg_min_sec(lambda_etrs, deg_lambda_etrs, min_lambda_etrs, sec_lambda_etrs);
    geographic2cartesian(phi_etrs, lambda_etrs, h_etrs, A_ETRS, INV_F_ETRS, x_etrs, y_etrs, z_etrs);
    sim_trans(x_etrs, y_etrs, z_etrs,
		      TX_ETRS_BESSEL, TY_ETRS_BESSEL, TZ_ETRS_BESSEL,
			  ALPHA_ETRS_BESSEL, BETA_ETRS_BESSEL, GAMMA_ETRS_BESSEL,
			  DELTA_ETRS_BESSEL,
			  x_amersfoort_etrs, y_amersfoort_etrs, z_amersfoort_etrs,
			  x_bessel, y_bessel, z_bessel);
    cartesian2geographic(x_bessel, y_bessel, z_bessel, A_BESSEL, INV_F_BESSEL, phi_bessel, lambda_bessel, h_bessel);
    decimal2deg_min_sec(phi_bessel, deg_phi_bessel, min_phi_bessel, sec_phi_bessel);
    decimal2deg_min_sec(lambda_bessel, deg_lambda_bessel, min_lambda_bessel, sec_lambda_bessel);
    rd_projection(phi_bessel, lambda_bessel, x_pseudo_rd, y_pseudo_rd);
    error_rd = rd_correction(x_pseudo_rd, y_pseudo_rd, x_rd, y_rd);
    error_nap = etrs2nap(phi_etrs, lambda_etrs, h_etrs, h_geoid);
    if (error_nap==3) nap=h_bessel;
    else nap=h_geoid;
	if (error_rd ==-1) return 0;
	if (error_nap==-1) return 0;
    ...
    **    Output
    **--------------------------------------------------------------
    ** stat_id is supplemented with spaces and printed with 22 characters
    */
	cout<<fixed<<setprecision(9);
    if (error_rd==0 && error_nap==0) stat_id.resize(stat_id.size()+22,' ');
    if (error_rd!=0 && error_nap==0) stat_id.resize(stat_id.size()+22,'-');
    if (error_rd==0 && error_nap!=0) stat_id.resize(stat_id.size()+22,'|');
    if (error_rd!=0 && error_nap!=0) stat_id.resize(stat_id.size()+22,'+');
    stat_id.resize (22);
    cout<<stat_id;
    cout<<" ETRS89 ";
 	cerr<<"decimal geographic coordinates"<<endl;
    cerr<<"    latitude           ";
	cout<<phi_etrs<<" ";
	cerr<<endl;
	cerr<<"    longitude           ";
	...
	cerr<<endl;
	cout<<setprecision(4);
	cerr<<"    ellipsoidal height ";
	cout<<h_etrs;
    cerr<<endl<<endl;

	cout<<fixed<<setprecision(5);
	cout<<"    ETRS89 ";
	cerr<<"sexagesimal geographic coordinates"<<endl;
	cerr<<"    latitude           ";
    printf ("%02d %02d %08.5f ", deg_phi_etrs, min_phi_etrs, sec_phi_etrs);
	cerr<<endl;
    cerr<<"    longitude           ";
    printf ("%02d %02d %08.5f ", deg_lambda_etrs, min_lambda_etrs, sec_lambda_etrs);
	cerr<<endl;
	cout<<setprecision(4);
	cerr<<"    ellipsoidal height ";
    printf ("%8.4f", h_etrs);
    ...
    cout<<"    ETRS89 ";
	cerr<<"cartesian coordinates"<<endl;
	cerr<<"    x                  ";
	cout<<x_etrs<<" ";
	cerr<<endl;
    cerr<<"    y                   ";
	...
	cerr<<"    x                  ";
	if (error_rd==0) cout<<x_rd<<" ";
	else cout<<x_rd<<" * ";
	cerr<<endl;
    cerr<<"    y                  ";
	if (error_rd==0) cout<<y_rd<<" ";
	else cout<<y_rd<<" * ";
	cerr<<endl;
	cerr<<"    NAP                ";
	if (error_nap==0) cout<<nap;
	else cout<<nap<<" * ";
	cerr<<endl<<endl;
	...
	cerr<<"sexagesimal geographic coordinates"<<endl;
	cerr<<"    latitude           ";
	cout<<deg_phi_bessel<<" "<<min_phi_bessel<<" "<<sec_phi_bessel<<" ";
	cerr<<endl;
    cerr<<"    longitude           ";
	cout<<deg_lambda_bessel<<" "<<min_lambda_bessel<<" "<<sec_lambda_bessel<<" ";
	cerr<<endl;
	cout<<setprecision(4);
	cerr<<"    ellipsoidal height ";
	cout<<h_bessel;
...
int main()
{
    ...
    string stat_id;
	double phi_etrs, lambda_etrs, h_etrs;
	double deg_phi_etrs, min_phi_etrs, sec_phi_etrs;
	double deg_lambda_etrs, min_lambda_etrs, sec_lambda_etrs;
	double x_etrs, y_etrs, z_etrs;

    double x_rd, y_rd, nap;
    double x_pseudo_rd, y_pseudo_rd;

	double phi_bessel, lambda_bessel, h_bessel;
	double deg_phi_bessel, min_phi_bessel, sec_phi_bessel;
	double deg_lambda_bessel, min_lambda_bessel, sec_lambda_bessel;
	double x_bessel, y_bessel, z_bessel;
    ...
    while (choice1!=0)
    {
        error=0;
        cerr<<endl;
        cerr<<"Coordinate system of input"<<endl;
        cerr<<"[1] ETRS89"<<endl;
        cerr<<"[2] RD and NAP"<<endl;
        cerr<<"[3] NL-Bessel"<<endl;
        cerr<<"[0] None; Close program"<<endl<<" ";
        cin>>choice1;
        if (choice1==1)
        {
            choice2=999;
			...
			{
                cerr<<endl;
                cerr<<"    Format of ETRS89 input"<<endl;
                cerr<<"    [1] decimal degrees"<<endl;
                cerr<<"    [2] sexagesimal degrees"<<endl;
                cerr<<"    [3] cartesian"<<endl;
                cerr<<"    [0] None; Back to main input menu"<<endl<<"     ";
                cin>>choice2;
                cerr<<" Geef de naam van het punt"<<endl;
                cin>>stat_id;
                if (choice2==1)
				{
			        cerr<<endl;
                    cerr<<"    Enter ETRS89 latitude (decimal degrees): ";
                    cin>>phi_etrs;
                    cerr<<"    Enter ETRS89 longitude (decimal degrees): ";
                    cin>>lambda_etrs;
                    cerr<<"    Enter ETRS89 ellipsoidal height (meters, enter 43 if unknown): ";
                    cin>>h_etrs;
                    cerr<<endl;

					output(stat_id, phi_etrs, lambda_etrs, h_etrs);
				}
                else if (choice2==2)
				{
			        cerr<<endl;
                    cerr<<"    Enter ETRS89 latitude (degrees): ";
                    cin>>deg_phi_etrs;
                    cerr<<"    Enter ETRS89 latitude (minutes): ";
                    cin>>min_phi_etrs;
                    cerr<<"    Enter ETRS89 latitude (seconds): ";
                    cin>>sec_phi_etrs;
                    cerr<<"    Enter ETRS89 longitude (degrees): ";
                    cin>>deg_lambda_etrs;
                    cerr<<"    Enter ETRS89 longitude (minutes): ";
                    cin>>min_lambda_etrs;
                    cerr<<"    Enter ETRS89 longitude (seconds): ";
                    cin>>sec_lambda_etrs;
                    cerr<<"    Enter ETRS89 ellipsoidal height (meters, enter 43 if unknown): ";
                    cin>>h_etrs;
                    cerr<<endl;

                    deg_min_sec2decimal(deg_phi_etrs, min_phi_etrs, sec_phi_etrs, phi_etrs);
                    deg_min_sec2decimal(deg_lambda_etrs, min_lambda_etrs, sec_lambda_etrs, lambda_etrs);
					output(stat_id, phi_etrs, lambda_etrs, h_etrs);
				}
                else if (choice2==3)
				{
			        cerr<<endl;
                    cerr<<"    Enter ETRS89 x (meters): ";
                    cin>>x_etrs;
                    cerr<<"    Enter ETRS89 y (meters): ";
                    cin>>y_etrs;
                    cerr<<"    Enter ETRS89 z (meters): ";
                    cin>>z_etrs;
                    cerr<<endl;

					cartesian2geographic(x_etrs, y_etrs, z_etrs, A_ETRS, INV_F_ETRS, phi_etrs, lambda_etrs, h_etrs);
					output(stat_id, phi_etrs, lambda_etrs, h_etrs);
				}
			}
		}
		else if (choice1==2)
        {
			...
			{
                cerr<<endl;
                cerr<<"    Type of RD and NAP input"<<endl;
                cerr<<"    [1] RD and NAP"<<endl;
                cerr<<"    [2] Pseudo RD and NAP"<<endl;
                cerr<<"    [0] None; Back to main input menu"<<endl<<"     ";
                cin>>choice2;
                cerr<<" Geef de naam van het punt"<<endl;
                cin>>stat_id;
				...
				{
			        cerr<<endl;
                    cerr<<"    Enter RD x (meters): ";
                    cin>>x_rd;
                    cerr<<"    Enter RD y (meters): ";
                    cin>>y_rd;
                    cerr<<"    Enter NAP (meters): ";
                    cin>>nap;
                    cerr<<endl;

                    error = rdnap2etrs(x_rd, y_rd, nap, phi_etrs, lambda_etrs, h_etrs);
                    output(stat_id, phi_etrs, lambda_etrs, h_etrs);
				}
                else if (choice2==2)
				{
					...
					error = rd_correction(x_pseudo_rd, y_pseudo_rd, x_rd, y_rd);
                    error = error + rdnap2etrs(x_rd, y_rd, nap, phi_etrs, lambda_etrs, h_etrs);
					if (error==0) output(stat_id, phi_etrs, lambda_etrs, h_etrs);
				}
			}
		}
		else if (choice1==3)
        {
			...
			{
                cerr<<endl;
                cerr<<"    Format of NL-Bessel input"<<endl;
                cerr<<"    [1] decimal degrees"<<endl;
                cerr<<"    [2] sexagesimal degrees"<<endl;
                cerr<<"    [3] cartesian"<<endl;
                cerr<<"    [0] None; Back to main input menu"<<endl<<"     ";
				...
				{
			        cerr<<endl;
                    cerr<<"    Enter NL-Bessel latitude (decimal degrees): ";
                    cin>>phi_bessel;
                    cerr<<"    Enter NL-Bessel longitude (decimal degrees): ";
                    cin>>lambda_bessel;
                    cerr<<"    Enter NL-Bessel ellipsoidal height (meters): ";
                    cin>>h_bessel;
                    cerr<<endl;

                    bessel2etrs(phi_bessel, lambda_bessel, h_bessel, phi_etrs, lambda_etrs, h_etrs);
					output(stat_id, phi_etrs, lambda_etrs, h_etrs);
				}
                else if (choice2==2)
				{
			        cerr<<endl;
                    cerr<<"    Enter NL-Bessel latitude (degrees): ";
                    cin>>deg_phi_bessel;
                    cerr<<"    Enter NL-Bessel latitude (minutes): ";
                    cin>>min_phi_bessel;
                    cerr<<"    Enter NL-Bessel latitude (seconds): ";
                    cin>>sec_phi_bessel;
                    cerr<<"    Enter NL-Bessel longitude (degrees): ";
                    cin>>deg_lambda_bessel;
                    cerr<<"    Enter NL-Bessel longitude (minutes): ";
                    cin>>min_lambda_bessel;
                    cerr<<"    Enter NL-Bessel longitude (seconds): ";
                    cin>>sec_lambda_bessel;
                    cerr<<"    Enter NL-Bessel ellipsoidal height (meters): ";
                    cin>>h_bessel;
                    cerr<<endl;

                    deg_min_sec2decimal(deg_phi_bessel, min_phi_bessel, sec_phi_bessel, phi_bessel);
                    deg_min_sec2decimal(deg_lambda_bessel, min_lambda_bessel, sec_lambda_bessel, lambda_bessel);
                    bessel2etrs(phi_bessel, lambda_bessel, h_bessel, phi_etrs, lambda_etrs, h_etrs);
					output(stat_id, phi_etrs, lambda_etrs, h_etrs);
				}
                else if (choice2==3)
				{
			        cerr<<endl;
                    cerr<<"    Enter NL-Bessel x (meters): ";
                    cin>>x_bessel;
                    cerr<<"    Enter NL-Bessel y (meters): ";
                    cin>>y_bessel;
                    cerr<<"    Enter NL-Bessel z (meters): ";
                    cin>>z_bessel;
                    cerr<<endl;
        ...
        {
            cerr<<endl;
            cerr<<"Regular end of program"<<endl;
            cerr<<endl;
        }

		//if ((choice1==1 || choice1==2 || choice1==3) && error==0)
		//{
		//cout<<phi_etrs<<endl;
		//}
    }
