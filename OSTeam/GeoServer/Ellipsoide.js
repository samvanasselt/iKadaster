...
var LambdaMin = -90;  //  Kleinste lengtegraad
...
function KijkHoek() {
    ...
    document.getElementById('kijkhoek-phi-toon').value    = phi;
    document.getElementById('kijkhoek-lambda-toon').value = lambda;
...
{//"class" cXYZ = Punt in de driedimensionale ruimte
    ...
    cXYZ.prototype.Zoom = function(pSchaal) {
        this.X *= pSchaal;
        this.Y *= pSchaal;
        this.Z *= pSchaal;
    }
...
function IsoMetrisch(lPunt) {
        var lKruisvak = document.getElementById('isometrisch');
        if (lKruisvak.checked) {
            var c = Math.sqrt(3) / 2;
            var s = 1.0 / 2;
...
{// class cEllipsoide
    ...
    cEllipsoide.prototype.TekenKromme = function(pKromme, pKijkHoek) {
        ...
        for (var i = 0; i < pKromme.Punten.length; i++) {
            ...
            lPunt.Zoom(document.getElementById("zoom").value);
    ...
    cEllipsoide.prototype.MaakParallellen = function(pPhiInterval, pLambdaInterval) {
        ...
        for (var phi = 50; phi <= 54; phi++) {
            this.Parallellen.push(new cKromme());
            for (var lambda = LambdaMin; lambda <= LambdaMax; lambda += pLambdaInterval) {
                this.Parallellen[this.Parallellen.length-1].PuntPL(phi,lambda);
            }
        }
    ...
    cEllipsoide.prototype.MaakMeridianen = function(pPhiInterval, pLambdaInterval) {
        ...
        for     (var lambda = 3; lambda <= 8; lambda++) {
            this.Meridianen.push(new cKromme());
            for (var phi    = PhiMin   ; phi    <= PhiMax   ; phi    += pPhiInterval) {
                this.Meridianen[this.Meridianen.length-1].PuntPL(phi,lambda);
            }
        }
    ...
    cEllipsoide.prototype.MaakNederland = function() {
        ...
        lRDpunten.push( new cRDpunt(236,618));  //  Rottermeroog
        ...
        var lRDpunten = [];  //  Rondom Amersfoort
        lRDpunten.push( new cRDpunt(154,463));
        lRDpunten.push( new cRDpunt(155,464));
        lRDpunten.push( new cRDpunt(156,463));
        lRDpunten.push( new cRDpunt(155,465));
        lRDpunten.push( new cRDpunt(154,463));
        this.Amersfoort = new cKromme();
        for (var i = 0; i < lRDpunten.length; i++) {
            lRDpunten[i].Schaal(1000);
            this.Amersfoort.Punt(RDnaarWGS84(lRDpunten[i]));
        }
        var lRDpunten = [];  //  Grenzen van het RD gebied
        lRDpunten.push( new cRDpunt(  0,300));  //  Linksonder
        lRDpunten.push( new cRDpunt(300,300));  //  Rechtsonder
        lRDpunten.push( new cRDpunt(300,625));  //  Rechtsboven
        lRDpunten.push( new cRDpunt(  0,625));  //  Linksboven
        lRDpunten.push( new cRDpunt(  0,300));  //  Linksonder
        this.RDkader = new cKromme();
        for (var i = 0; i < lRDpunten.length; i++) {
            lRDpunten[i].Schaal(1000);
            this.RDkader.Punt(RDnaarWGS84(lRDpunten[i]));
        }
    }
    cEllipsoide.prototype.TekenParallellen = function(pKijkHoek) {
        for (var i = 0; i < this.Parallellen.length; i++) {
            var lBreedte = this.Parallellen[i].Punten[0].Phi;
            var lEvenaar   = (lBreedte == 0);
            var lNederland = (lBreedte >= 50 && lBreedte <= 54);
                 if (lEvenaar  ) gEllipsCanvas.strokeStyle = 'rgb(255, 63, 63)';    //  Evenaar rood
            else if (lNederland) gEllipsCanvas.strokeStyle = 'rgb(195,195,195)';    //  Nederland lichtgrijs
            else                 gEllipsCanvas.strokeStyle = 'rgb(127,127,127)';    //  Overige parallellen grijs
    ...
    cEllipsoide.prototype.TekenMeridianen  = function(pKijkHoek) {
        for (var i = 0; i < this.Meridianen.length ; i++) {
            var lLengte = this.Meridianen[i].Punten[0].Lambda;
            var lNulMeridiaan = (lLengte == 0);
            var lNederland    = (lLengte >= 3 && lLengte <= 8);
                 if (lNulMeridiaan) gEllipsCanvas.strokeStyle = 'rgb(255, 63, 63)'; //  Nul-meridiaan rood
            else if (lNederland   ) gEllipsCanvas.strokeStyle = 'rgb(195,195,195)'; //  Nederland lichtgrijs
            else                    gEllipsCanvas.strokeStyle = 'rgb(127,127,127)'; //  Overige meridianen grijs
    ...
    cEllipsoide.prototype.TekenNederland   = function(pKijkHoek) {
        ...
        this.TekenKromme(this.Amersfoort, pKijkHoek);
        gEllipsCanvas.strokeStyle = 'rgb(0,0,127)';
        this.TekenKromme(this.RDkader, pKijkHoek);
    }
}
{// class cAssenStelsel
...
function ZoomTo(pZoomfactor) {
    document.getElementById('zoom-toon').value = pZoomfactor;
    WisCanvas();
    TekenAarde();
}
function TekenAarde() {
    var a = document.getElementById('ellips-lange-as').value;
    var b = document.getElementById('ellips-korte-as').value;
    var lHoek = KijkHoek();
    lAarde = new cEllipsoide(a,b);
